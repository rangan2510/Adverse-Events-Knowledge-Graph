...
DATASETS: dict[str, Dataset] = {
    ...
    "opentargets": Dataset(
        "opentargets", "Open Targets Platform", ["hgnc"], has_normalize=True, tier=2
    ),
    "reactome": Dataset(
        "reactome", "Reactome Pathways", ["hgnc"], has_normalize=True, tier=2
    ),
    "gtop": Dataset(
        "gtop", "Guide to Pharmacology", ["hgnc", "drugcentral"], tier=2
    ),
    
    # Tier 3: Associations (depend on Tier 1-2)
    "sider": Dataset(
        "sider", "SIDER Drug-ADR", ["drugcentral"], has_normalize=True, tier=3
    ),
    "openfda": Dataset("openfda", "openFDA FAERS", ["drugcentral"], tier=3),
    "ctd": Dataset(
        "ctd", "CTD Toxicogenomics", ["hgnc", "drugcentral"], tier=3
    ),
    ...
    "chembl": Dataset(
        "chembl", "ChEMBL Bioactivity", ["hgnc", "drugcentral"], tier=4
    ),
...
class ETLRunner:
    ...
    def _build_dashboard(self, current_dataset: str | None = None) -> Panel:
        ...
        return Panel(
            content, title="[bold blue]Drug-AE Knowledge Graph ETL[/]", border_style="blue"
        )
        ...
        with Live(self._build_dashboard(), console=self.console, refresh_per_second=4) as live:
            for ds_key in datasets_to_run:
                ...
                for phase in ds_phases:
                    ...
                    if not success:
                        self.console.print(
                            f"[red]Pipeline stopped: error in {ds_key}/{phase}[/]"
                        )
                        return False
    ...
    def run_tier(self, tier: int, force: bool = False) -> bool:
        ...
        with Live(
            self._build_dashboard(), console=self.console, refresh_per_second=4
        ) as live:
    ...
    def run_interactive(self) -> None:
        ...
        while True:
            ...
            if choice == "q":
                ...
                self.console.print(
                    "  [cyan]3[/] - Associations (SIDER, openFDA, CTD, STRING, ...)"
                )
