...
class SiderNormalizer(BaseNormalizer):
    ...
    def _normalize_adverse_events(self, side_effects: pl.DataFrame) -> Path:
        ...
        ae_terms = (
            side_effects.filter(pl.col("meddra_type") == "PT")  # Prefer preferred terms
            .select(
                [
                    "umls_cui_meddra",
                    "side_effect_name",
    ...
    def _normalize_drug_ae_pairs(self, side_effects: pl.DataFrame, frequencies: pl.DataFrame) -> Path:
        ...
        pairs_base = (
            side_effects.filter(pl.col("meddra_type") == "PT")  # Use preferred terms
            .select(
                [
                    "stitch_id_flat",
                    "umls_cui_meddra",
        ...
        freq_data = (
            frequencies.filter(pl.col("meddra_type") == "PT")
        ...
        pairs = (
            ...
            .with_columns(
                [
                    ...
                    ((pl.col("freq_lower_avg") + pl.col("freq_upper_avg")) / 2).alias("frequency_score"),
                ]
            )
            .select(
                [
