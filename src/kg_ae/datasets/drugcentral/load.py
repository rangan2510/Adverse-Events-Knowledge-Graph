...
class DrugCentralLoader(BaseLoader):
    ...
    def load(self) -> dict[str, int]:
        ...
        """
        console.print("[bold cyan]DrugCentral Loader[/]")
        results = {}
        ...
        # Summary table
        table = Table(title="DrugCentral Load Summary", show_header=True)
    ...
    def _load_drugs(self, dataset_id: int) -> int:
        ...
        if not drugs_path.exists():
            console.print("  [dim][skip] No drugs.parquet found[/]")
            return 0
        ...
        with Progress(
            ...
            task = progress.add_task("Loading drugs", total=len(df))
        ...
        console.print(f"    [green]✓[/] Loaded drugs: {count:,}")
        return count
    ...
    def _load_genes(self, dataset_id: int) -> int:
        ...
        with Progress(
            ...
            task = progress.add_task("Loading genes", total=len(df))
            for row in df.iter_rows(named=True):
        ...
        console.print(f"    [green]✓[/] Loaded genes: {count:,}")
        return count
    ...
    def _load_interactions(self, dataset_id: int) -> int:
        ...
        with Progress(
            ...
            task = progress.add_task("Loading interactions", total=len(df))
            for row in df.iter_rows(named=True):
                drugcentral_id = row.get("drugcentral_id")
        ...
        console.print(f"    [green]✓[/] Loaded interactions: {count:,}")
        return count
