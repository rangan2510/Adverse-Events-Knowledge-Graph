...
Build and export subgraphs from the knowledge graph (GraphStore-backed).
"""
...
class Subgraph:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            "nodes": [{"id": n.id, "type": n.type, "label": n.label, **n.properties} for n in self.nodes],
            "edges": [
                {"source": e.source, "target": e.target, "type": e.type, "weight": e.weight, **e.properties}
                for e in self.edges
            ],
        }
    ...
    def to_cytoscape(self) -> dict:
        ...
        elements = []
        for n in self.nodes:
            elements.append({"data": {"id": n.id, "label": n.label, "type": n.type, **n.properties}, "group": "nodes"})
        for e in self.edges:
            elements.append(
                {
                    "data": {
...
def build_subgraph(
    ...
    """Build a subgraph centered on the given drugs."""
    ...
    for drug_key in drug_keys:
        ...
        add_node(drug_id, "Drug", store.node_label("Drug", drug_key))

        if include_targets:
            for t in get_drug_targets(drug_key):
        ...
        if include_aes:
            for ae in get_drug_adverse_events(drug_key, limit=max_aes_per_drug):
    ...
    if include_pathways:
        for gene_key in gene_keys:
            ...
            for pw in get_gene_pathways(gene_key)[:max_pathways_per_gene]:
    ...
    if include_diseases:
        for gene_key in gene_keys:
            gene_id = f"gene:{gene_key}"
            for dis in get_gene_diseases(gene_key, min_score=min_disease_score)[:max_diseases_per_gene]:
...
def score_edges(graph: Subgraph, weights: dict[str, float] | None = None) -> Subgraph:
    """Apply evidence-based scoring to graph edges by edge type."""
    default_weights = {
    ...
    for edge in graph.edges:
        type_weight = weights.get(edge.type, 0.5)
        edge.weight = edge.weight * type_weight
    return graph
